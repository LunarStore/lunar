...
namespace lunar {
    namespace http {
        ...
        #define PARSE_PARAM(str, m, flag, trim) \
            ...
            do { \
                ...
                m.insert(std::make_pair(trim(str.substr(last, key - last)), \
                            lunar::StringUtil::UrlDecode(str.substr(key + 1, pos - key - 1)))); \
                if(pos == std::string::npos) { \
                    break; \
                } \
                ++pos; \
        ...
        void HttpRequest::initCookies() {
            ...
            PARSE_PARAM(cookie, m_cookies, ';', lunar::StringUtil::Trim);
            m_parserParamFlag |= 0x4;
        }
        ...
        void HttpResponse::setCookie(const std::string& key, const std::string& val,
            ...
            ss << key << "=" << val;
            if(expired > 0) {
                ss << ";expires=" << lunar::Time2Str(expired, "%a, %d %b %Y %H:%M:%S") << " GMT";
            }
            if(!domain.empty()) {
                ss << ";domain=" << domain;
            }
