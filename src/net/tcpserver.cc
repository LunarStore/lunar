...
TcpServer::TcpServer(lunar::IOManager* worker,
...
bool TcpServer::bind(lunar::Address::ptr addr, bool ssl) {
...
bool TcpServer::bind(const std::vector<Address::ptr>& addrs
    ...
    for(auto& addr : addrs) {
        ...
        if(!sock->bind(addr)) {
            LUNAR_LOG_ERROR(g_logger) << "bind fail errno="
        ...
        if(!sock->listen()) {
            LUNAR_LOG_ERROR(g_logger) << "listen fail errno="
                << errno << " errstr=" << strerror(errno)
                << " addr=[" << addr->toString() << "]";
            fails.push_back(addr);
            continue;
    ...
    for(auto& i : m_socks) {
        LUNAR_LOG_INFO(g_logger) << "type=" << m_type
            << " name=" << m_name
            << " ssl=" << m_ssl
...
void TcpServer::startAccept(Socket::ptr sock) {
    while(!m_isStop) {
        ...
        if(client) {
            ...
            LUNAR_LOG_ERROR(g_logger) << "accept errno=" << errno
